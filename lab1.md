...
> **Solution adopted:** option 1 — local remote. The DagsHub `dvc push`
> repeatedly failed with dropped connections and socket timeouts while
> uploading ~13,000 image files, so the DVC remote was set to a local
...
> Answers:
  ...
  >> .dvc/config and .dvc/.gitignore and .dvcignore were pushed to github
...
> Answers:
  >> yes 
  >> it contains the md5 hash and acts as the data pointer that gets tracked by git
